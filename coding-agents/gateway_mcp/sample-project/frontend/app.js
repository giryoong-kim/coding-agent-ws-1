...
function renderTasks(tasks) {
    ...
    // BUG: uses innerHTML += in a loop, re-parsing the entire DOM each iteration and losing event listeners
    list.innerHTML = "";
...
function formatDate(isoString) {
    // BUG: doesn't handle null/undefined, so it crashes if created_at is missing
    const date = new Date(isoString);
    return date.toLocaleDateString();
}

function cycleStatus(taskId, currentStatus) {
    // BUG: status cycle is wrong, going todo -> done, skipping "doing"
...
function showError(msg) {
    ...
    // BUG: error never gets hidden, so it stays visible forever after the first error
}
