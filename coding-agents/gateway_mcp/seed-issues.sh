...
gh issue create --repo "${OWNER}/${REPO}" \
  ...
  --body "The filter in \`delete_task\` uses \`t['id'] == task_id\` (keeps matching) instead of \`t['id'] != task_id\` (keeps non-matching). This inverts the logic, so calling delete removes everything except the task you wanted to delete." \
  --label "bug"
