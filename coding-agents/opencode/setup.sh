#!/usr/bin/env bash
# Build and push the opencode (PTY) container image to ECR.
set -euo pipefail
...
ECR_REPO="coding-agents-opencode"
IMAGE_TAG="latest"
...
if [ -n "${WORKSHOP_PUBLIC_IMAGE_URI:-}" ]; then
  echo "=============================================="
  echo "  opencode (PTY): PUBLIC ECR mirror (no build)"
...
# derived from this URI by deploy.py. Falls back to a per-account build when unset.
  ...
  echo "=============================================="
  echo "  opencode (PTY): PREBUILT (central ECR, no build)"
  echo "  Image: ${ECR_URI}"
  echo "=============================================="
else
  echo "=============================================="
  echo "  opencode (PTY): Build & Push"
