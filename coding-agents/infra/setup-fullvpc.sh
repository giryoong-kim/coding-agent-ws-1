#!/usr/bin/env bash
# APPENDIX (own-account path): create EVERYTHING from scratch in one shot: S3
# bucket + VPC + S3 Files file system/access point/mount targets, then write
# infra.config and upload the skill.
#
# In the GUIDED workshop you do NOT run this: the VPC is pre-provisioned and you
# create the S3 Files access point by hand (that is the teaching step), then run
# ./setup.sh to record the values. Use THIS script only when you are running the
# workshop in your own account and want the shared infra stood up automatically.
set -euo pipefail
